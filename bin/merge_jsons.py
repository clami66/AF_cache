...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Merges multiple inputs to AF3 (json files) into a single, multichain input. Chains are named according to the order of json files submitted by the user"
    )
    parser.add_argument(
        "--jsons", nargs="+", help="Path to multiple AF3 inputs (json files)"
    )
