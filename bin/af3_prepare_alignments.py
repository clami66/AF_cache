    ...
    with Pool(processes=int(args.n_cpu)) as pool:
        pool.map(
            partial(
                ...
                custom_taxids=args.custom_taxids,
                use_taxid=args.taxid,
            ),
            a3ms,
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Converts an a3m alignment (e.g. MMseqs2 alignment) to a Stockholm alignment \
    ...
    parser.add_argument(
        "--taxid",
        help="Use tax ID rather than representative ID for pairing",
        action="store_true",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Space-separated file of taxIDs or mnemoic IDs to replace in the outputs (e.g. '12345 6789' to convert taxID 12345 to 6789), for example in case to replace virus taxIDs with their respective host taxIDs",
    )
